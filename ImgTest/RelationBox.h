...
public:
	...
	{
		//向量第一位（at(0)）存储邻域个数，第二位（at(1)）存储色调（H）均值，第三位（at(2)）存储饱和度（S）均值，
		//第四位（at(3)）存储区域内像素个数，第五位（at(4)）存储标号位，第六位（at(5)）存储周长，
		//第七位（at(6)）存储ban,取值： 0（默认）--不ban，1--ban，被判定为肯定不是房屋，之后存储邻域们的区域标号
				...
				if (marks.at<int>(i, j) == -1)
				{
					...
					if ((numWshed > 0) && (numWshed < 3)) //如果这个像素的邻域中只有一个或者两个像素属于分水岭，它两侧可能是两个分割区域
					{
						if (neigZones.size() == 2) //如果这个像素的邻域中有且仅有两个归属分割区域
						{
							...
							if (relationMatrix[container0].size() == persize) //这个[container0 - 1]向量还没添加过邻域信息
							{
							...
							else if (relationMatrix[container0].size() > persize) //这个向量添加过邻域信息
							{
								for (int k = persize; k < relationMatrix[container0].size(); ++k) //从第六位开始遍历
								{
							...
							if (relationMatrix[container1].size() == persize) //这个[container1 - 1]向量还没添加过邻域信息
							{
							...
							else if (relationMatrix[container1].size() > persize) //这个向量添加过邻域信息
							{
								for (int k = persize; k < relationMatrix[container1].size(); ++k) //从第六位开始遍历
								{
